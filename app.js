(function () {
  ...
  // Normalize subject/topic/difficulty once so stray whitespace or casing in the
  // data can never cause a subject to silently end up with zero matching topics.
  bank.forEach(function (q) {
    q.subject = (q.subject == null ? "" : String(q.subject)).trim();
    q.topic = (q.topic == null ? "" : String(q.topic)).trim();
    q.difficulty = (q.difficulty == null ? "" : String(q.difficulty)).trim();
  });

  document.getElementById("bank-total").textContent = bank.length;
  var tickerTotalEls = document.querySelectorAll(".ticker-total-num");
  for (var ti = 0; ti < tickerTotalEls.length; ti++) tickerTotalEls[ti].textContent = bank.length;

  var subjects = uniq(bank.map(function (q) { return q.subject; }).filter(Boolean)).sort();
  ...
  var stickySelectionEl = document.getElementById("sticky-selection");
  var stickyHintMirrorEl = document.getElementById("sticky-hint-mirror");
  ...
  function updateAvailableHint() {
    ...
    if (stickySelectionEl) {
      var parts = [state.subject === "All" ? "All subjects" : state.subject];
      if (state.subject !== "All" && state.topic !== "All") parts.push(state.topic);
      if (state.difficulty !== "All") parts.push(state.difficulty);
      stickySelectionEl.textContent = parts.join(" · ");
    }
    if (stickyHintMirrorEl) {
      stickyHintMirrorEl.textContent = n + " question" + (n === 1 ? "" : "s") + " available";
    }
  }
